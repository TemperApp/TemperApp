...
const PitchNote: React.FC<PitchNoteProps> = ({ active, name, state, frequency, mode, noteBuffer, onSetBuffer, onChange }) => {
  ...
  const NoteClassName = () => {
      ...
      if( (mode === TunerMode.TuningFork && (state.note.name === name.name && state.note.octave === name.octave)) || 
          (mode === TunerMode.Bpm && ((state.note.name === name.name && state.note.octave === name.octave) || (name.name === noteBuffer.noteBpm1.name && name.octave === noteBuffer.noteBpm1.octave ) || (name.name === noteBuffer.noteBpm2.name && name.octave === noteBuffer.noteBpm2.octave )))
      ){
